    ...
    switch($report) {
      ...
      case /html/ {
        ...
        for my $key (@keys) {
          ...
          my $row;
          switch($key->{id}) {
            case /B002/ { print $fh "<h3><a id=\"beck\"/>Beck Archives</h3>\n"; }
            case /D001/ { print $fh "<h3><a id=\"dance\"/>Carson Brierly Giffen Dance Library</h3>\n"; }
            case /M001/ { print $fh "<h3><a id=\"mss\"/>Manuscripts</h3>\n"; }
